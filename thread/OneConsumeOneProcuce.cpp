...
void ProduceItem(ItemRepository *ir, int item)
{
	...
	while (((ir->write_position + 1) % kItemRepositorySize)== ir->read_position) { // item buffer is full, just wait here.   写位置多一圈并在读位置后一位
		std::cout << "Producer is waiting for an empty slot...\n";
		(ir->repo_not_full).wait(lock); //阻塞线程 直到另外一个线程在相同的std::condition_variable对象上调用了notification函数来唤醒当前线程	
		//唤醒需要相同的std::condition_variable
	}
...
int ConsumeItem(ItemRepository *ir)
{
	...
	if (ir->read_position >= kItemRepositorySize) // 读取位置若移到最后，则重新置位.
		ir->read_position = 0;
	(ir->repo_not_full).notify_all(); // 通知消费者产品库不为满.s
	return data; // 返回产品.
}
